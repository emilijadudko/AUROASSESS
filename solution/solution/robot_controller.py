...
class RobotController(Node):
    ...
    def item_callback(self, item_list):
        if item_list.items:
        # Example: Select the closest item as the target
            self.target_item = min(
            item_list.items,
            key=lambda item: math.sqrt(item.position.x**2 + item.position.y**2)
        )
        if self.state != State.MOVE_TO_ITEM:
            self.get_logger().info(f"Target item detected: {self.target_item.name}")
            self.state = State.MOVE_TO_ITEM
        else:
            self.target_item = None
        if self.state != State.FORWARD:
            self.get_logger().info("No items detected. Moving forward.")
            self.state = State.FORWARD
    ...
    def control_loop(self):
        ...
        match self.state:
            ...
            case COLLECTING:
                self.get_logger().info("collecting item")
                self.get_logger().info(f"collected item: {self.target_item.name}")
                
                self.target_item = None
                self.state = FORWARD
                self.get_logger().info("switching to forward state")
                # when item hass been picked up
